	...
	inherit_resources
  	...
  	def create
  		if params[:clients_list]
  			...
  			flash.now[:info] = "Processed: #{@processed_count} Unsaved: #{@unsaved_count}"
  			render 'new' and return unless @unprocessed_lines.blank?
  			redirect_to new_client_path, flash:{info: "Processed: #{@processed_count} Unsaved: #{@unsaved_count}"} and return
  		end
