class Survey < ActiveRecord::Base
  attr_accessible :account_id, :description, :name, :user_id, :user, :account
  ...
  def sorted_items(params = {})
    self.items.sort{|x1, x2| self.items_positions.index(x1.id) <=> self.items_positions.index(x2.id)}
  end
